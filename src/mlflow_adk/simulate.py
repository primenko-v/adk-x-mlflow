...
def _select_paths(input_dir: Path, paths: list[Path], select: list[str]) -> list[Path]:
    """Keep only ``paths`` whose name matches one of the glob ``select`` patterns.

    A pattern is matched against two forms of each file, so the same name works
    whether you think in cases or in folders:

    - the bare stem (the EvalCase ``eval_id``) — ``curious_traveler``,
      ``temperature_*`` select a case regardless of which subdirectory it sits in;
    - the path relative to ``input_dir``, without the ``.yaml`` suffix —
      ``static/seeded_fahrenheit`` or ``scenarios/*`` select by folder.

    A pattern ending in ``/`` is shorthand for "everything under this folder"
    (``static/`` ≡ ``static/*``). A pattern that matches nothing is an error —
    almost always a typo, and silently running fewer cases than asked is worse
    than a clear failure.
    """
    rels = {p: p.relative_to(input_dir).with_suffix("").as_posix() for p in paths}
    chosen: list[Path] = []
    unmatched: list[str] = []
    for pattern in select:
        ...
        hits = [
            p
            for p in paths
            if fnmatch.fnmatch(p.stem, pat) or fnmatch.fnmatch(rels[p], pat)
        ]
        if not hits:
            unmatched.append(pattern)
        chosen.extend(hits)
    if unmatched:
        available = ", ".join(sorted(rels.values())) or "(none)"
        raise ValueError(
            f"--select pattern(s) matched no conversation: {', '.join(unmatched)}. "
            f"Available cases: {available}"
        )
    # De-dup while preserving order: overlapping patterns must not run a case twice.
    seen: set[Path] = set()
    return [p for p in chosen if not (p in seen or seen.add(p))]


def load_simulation_list(path: Path) -> list[str]:
    """Read glob patterns (one per line) from a simulation-list file.

    Blank lines and ``#`` comments are ignored. Each remaining line is a glob
    pattern fed to ``load_eval_set(select=...)`` to pick which conversations
    run. A ``*`` line runs everything. The file is the make-friendly stand-in
    for typing patterns on the CLI — edit ``simulations/default.txt`` instead.
    """
    patterns = []
...
def load_eval_set(input_dir: Path, select: list[str] | None = None) -> EvalSet:
    """Load ``*.yaml`` under ``input_dir`` (recursively) as EvalCases.
    ...
    ``select`` is an optional list of glob patterns matched against each file's
    stem and its path relative to ``input_dir`` (see ``_select_paths``); only
    matching cases are loaded. ``None`` (the default) loads every file found.
    """
    paths = sorted(input_dir.rglob("*.yaml"))
    if select:
        paths = _select_paths(input_dir, paths, select)
    cases = [_build_eval_case(path) for path in paths]
...
async def run_simulation(
    ...
    ``select`` is an optional list of glob patterns; only conversations whose
    filename stem matches one are run (e.g. ``["curious_traveler"]`` or
    ``["temperature_*"]``). Default ``None`` runs every case found.
    ...
    eval_set = load_eval_set(input_dir, select=select)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        dest="simulation_list",
        help=(
            "Path to a file of glob patterns (one per line, '#' comments ok). "
            "Only conversations matching a pattern run. Patterns match the case "
            "stem ('curious_traveler', 'temperature_*') or the path under the "
            "conversations dir ('static/seeded_fahrenheit', 'static/'). A '*' "
            "line runs all; omit the flag to run all."
        ),
    )
    parser.add_argument(
        "--output-traces",
    ...
    select = (
        load_simulation_list(args.simulation_list) if args.simulation_list else None
    )
    asyncio.run(
        run_simulation(
            args.input_dir,
